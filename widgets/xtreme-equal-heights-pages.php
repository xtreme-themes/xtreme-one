...
add_filter('xtreme-collect-widget-classes', create_function('$classes', '$classes[] = "Xtreme_Equalize_Pages_Widget"; return $classes;'));

class Xtreme_Equalize_Pages_Widget extends Xtreme_Widget_Base {

	function __construct() {
		$widget_ops = array(
			'classname' => 'xtreme_equalize_pages',
				...
				while ( $r->have_posts() ) : $r->the_post();
					...
					if(isset($new_sort)) {
						...
						$link[] = array(
							'url' => get_permalink(),
							'title' => get_the_title(),
							'read_more' => esc_html($instance['excerpt_morelink_text-'.$i])
						);
					}
				endwhile;
				wp_reset_query();
...
<?php endif; ?>
	...
	function html_before( $columns, $count, $box_bottom ) {
		...
		$this->no_bottom = "";
		if ( 0 === $box_bottom ) {
			$this->no_bottom = "no-bottom";
		}
		$html = "";

		if ( $this->count % $this->columns == 1 ) {
			$html .= "<div class=\"ym-grid ym-equalize linearize-level-2 box-top " . $this->no_bottom . "\">\n";
		}
		$html .= "<div class=\"" . $this->classes[$this->columns][$this->count-1]['outer'] . "\">\n";
		$html .= "<div class=\"" . $this->classes[$this->columns][$this->count-1]['inner'] . "\">\n";
		return $html;
	}

	function html_after( $columns, $count, $box_bottom, $link ) {
		$this->columns = ( int ) $columns;
		$this->count = ( int ) $count;
		$this->use_box_bottom = $box_bottom;
		$html = "";
		if ( $this->count % $this->columns == 0 ) {
			$html .= "</div>\n</div>\n</div>\n";
			if ( 1 === $this->use_box_bottom ) {
				$val = $this->columns * ( ( $this->count / $this->columns ) -1 );
				$html .= "<div class=\"ym-grid ym-equalize linearize-level-2 no-ie-padding box-bottom\">\n";
				for ( $x = 0; $x < $this->columns; $x++ ) {
					$html .= "  <div class=\"" . $this->classes[$this->columns][$x % $this->columns]['outer'] . "\">\n";
					$html .= "    <div class=\"" . $this->classes[$this->columns][$x % $this->columns]['inner'] . "\">\n";
					$html .= "<span class='read-more'><a href=\"" . esc_url( $link[( $x + $val )]['url']) . "\" title=\"" . esc_attr( __( 'Read more about', XF_TEXTDOMAIN ) .  " " . $link[( $x + $val )]['title'] ) . "\">". esc_attr( $link[( $x + $val )]['read_more'] ) . "</a></span>";
					$html .= "    </div>\n";
					$html .= "  </div>\n";
				}
				$html .= "</div>\n";
